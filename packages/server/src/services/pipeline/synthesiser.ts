...
export async function synthesise(userId: string, graph: EntityGraph, condensedText?: string): Promise<Profile> {
  const client = createClient();
  const prompt = buildSynthesisPrompt(graph, userId, condensedText);
